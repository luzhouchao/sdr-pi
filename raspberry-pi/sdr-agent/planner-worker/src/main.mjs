...
const server = createServer((socket) => {
  ...
  socket.on("data", (chunk) => {
    ...
    if (busy) {
      writeResponse(socket, makeErrorResponse(undefined, plannerMeta, "unavailable", "planner is busy"));
      return;
    }
    busy = true;
...
async function handleFrame(frame) {
  ...
  const submitPlan = {
    ...
    execute: async (_toolCallId, params) => {
      ...
      submittedPlans.push(normalizeAction(params));
      return {
  ...
  const agent = new Agent({
    ...
    sessionId: `sdr-${request.session_generation}`,
