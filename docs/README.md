...
原2018A实测流程继续有效。四倍RRC是新增数据集的传输候选，用户同意给2018A另加同流程配对先导；没有替换原流程或全量重跑。四套2272固定seed42源窗的离线同步/相消/存储前端通过，48项回归通过、1 CUDA跳过，尚未实机收发。新方法SINR合同仍待验证，见[统一候选与原基线边界](reference/RML2018A_RF_REPRODUCTION.md#611-四套rrc候选的离线同步与相消接入2026-09-26)。

当前目标为RML2016A、RML2016B、HisarMod2019三套D10的发射→接收→处理→识别，沿用原RML2018A流程原则；尚未开始新收发。用户已确认四数据集×source/接收原始IQ/raw/guard共16套数据，模型单独管理；新增要求统一四数据集×八种模型共32份权重。新增三套先导固定352/320/832个seed42验证成员，2018A补768窗配对先导；扩大范围尚未冻结。
