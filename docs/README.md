...
最新新seed对照：随机滚降/时间偏移未修复主要误判，3200 D10输入完成，700416已知符号零错；停止继续凭准确率试参数，见[完整四组结果与边界](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#新种子随机滚降与时间偏移对照2026-09-26)。无RF/训练/部署。

最新物理核验：自生成数字波形175104符号零错、最差EVM0.50%，AM构造检查通过；正确波形仍不等于训练分布匹配。原论文含随机滚降/时间偏移等，本地精确生成参数未定；见[核验与资料依据](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#自生成波形物理核验与生成依据2026-09-26)。无新推理/RF。

最新参数核对：BPSK改为2点/符号后64/64，QPSK在8点/符号63/64而其余档为0；16QAM/AM仍未解决。支持生成参数敏感，未确定原数据集参数或实测根因；见[固定网格对照](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#自生成信号符号率与相位对照2026-09-26)。

最新离线合成对照：五类1024点波形完成20,800次D10预测；干净仅QPSK63/64，其他四类0。
精确相消仍有局部识别回退，但低SINR总体改善，不能据此认定实测根因；见[结果与生成分布限制](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#自生成1024点可控相消对照2026-09-26)。无RF/训练/部署。
