...
用户最新要求四个数据集采用同一套主实验方案，2018A也参加；旧2018A结果保留为历史基线。统一RF参数、物理帧时长、同步/相消和SINR口径，保留原生128/1024点输入。先做共同profile的四套有限先导，不直接全量重跑，见[统一主实验合同](reference/RML2018A_RF_REPRODUCTION.md#612-四数据集统一主实验要求用户最新选择2026-09-26)。现有RRC候选48项回归通过、1 CUDA跳过，但共同物理帧及新SINR合同仍待验证，尚未新RF。

当前目标为RML2016A、RML2016B、RML2018A、HisarMod2019在统一profile下完成发射→接收→处理→八模型识别；尚未开始新收发。保留四数据集×source/接收原始IQ/raw/guard共16种逻辑数据，四数据集×八模型共32份权重单独管理。四套有限先导分别352/320/768/832个seed42验证成员，拟统一Z18；新预登记和扩大范围仍待完成。
