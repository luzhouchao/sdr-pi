...
四数据集×八模型32份seed42权重已导入。夜间统一RRC短帧实收2272/2272窗同步，P201缓冲4096→65536消除本次先导的大时间跳变；2016A/B模型入口恢复原Σ|IQ|=1约定，四套D10相消后89.32%～95.43%。见[夜间统一RF实测](validation/AMC_UNIFORM_RF_2026-09-26.md)；仍为有限工程验证，不宣称低SINR问题已解决或全量/生产准入。

9月27日用户已授权推进四套**完整seed42验证集**：2016A→2016B→2018A→Hisar，共713385行；当前套完整接收、落盘/SHA封存后才启动下一套。后台任务已启动，完成情况以[当前执行选择](SDR_AGENT_ACTUAL_DELIVERY_ORDER_2026-09-06.md#当前执行选择)及应用结果根`validation-seed42-20260927/progress.json`为准，不将启动视为全套完成。
