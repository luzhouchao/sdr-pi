...
随后[Web 升级与恢复验收](validation/WEB_RECOVERY_UPGRADE_VALIDATION_2026-09-07.md)
已实际安装当前 Web/归档界面并验证回滚；生产识别系统仍未整体上线。Worker、
校准/profile 和准入配置未部署，A1 保持未完成。下文原隔离验证的部署描述保留
当时时点，不应据此重新施工已经完成的入口或 Web 单元。
...
- [x] 按用户指定的 4090 原始标签表冻结 RML2018A 数字 ID→名称映射；源码、
      Y.argmax Adapter 语义及报告 24/24 核对通过，见
      [server-v1 记录](validation/RML2018A_LABEL_MAPPING_VALIDATION_2026-09-08.md)。
      生产 name_evidence 接入属于 A1，未准入运行标签及历史记录不改写。
...
2026-09-08 用户明确暂缓第7章“辐射源/设备身份识别”，其条目已从当前待办
移出，不计作未完成交付，也不标记为已验收。恢复须有用户新的明确指令。
当前聚焦第1—6章 RX-only 闭环；调制识别 checkpoint 不证明发射设备身份。
