...
| 接下来 | 独立单元 | 完成边界 |
| --- | --- | --- |
| 1 | 非模型运维配置部署 | 核对已安装 Web health 与实际运行 Planner 的健康接口兼容性，单独安装和验证已有 O1a 日志轮转、只读健康与本地去重告警；用户结果不参与日志或升级清理 |

Web/RX 入口单元不要重做；下一单元继续按实际安装状态限定依赖和范围，测试、
清理、提交并立即 push。当前识别接口存在不构成部署 Worker、训练或放开模型
能力的授权。
