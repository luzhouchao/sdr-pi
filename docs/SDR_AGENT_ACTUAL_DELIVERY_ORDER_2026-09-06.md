...
2026-09-09用户明确恢复非模型的50Ω负载物理排查，先做停发负载背景，见
[本轮验证](validation/P201_TERMINATION_BACKGROUND_VALIDATION_2026-09-09.md)。该独立
单元之后，须取得用户实际换线确认，才能按固定参数登记并执行同会话天线
对照；不能把跨日历史比较记作完整隔离，也不由此恢复发射或模型实验。
