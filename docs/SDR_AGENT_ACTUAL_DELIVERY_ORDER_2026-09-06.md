...
负载及接回原天线的顺序比较，见[本轮验证](validation/P201_TERMINATION_BACKGROUND_VALIDATION_2026-09-09.md)。
如继续隔离，下一独立单元应以再次实际换线确认后的反向对照检查可重复性；
不能把目前两阶段比较记作完整三阶段隔离，也不由此恢复发射或模型实验。
