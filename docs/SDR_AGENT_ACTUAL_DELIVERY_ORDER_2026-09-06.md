...
用户于2026-09-10明确“以后都用433”。后续实验收发默认 **433.920MHz**，使用当前
433MHz天线；沿用该选择，不再自行切回2.4/3.5/5GHz，也不因新对话重复询问频段。
具体更换端和天线型号尚未报告，不能补成已核对的产品参数。

已验证的单音参考组合：TX70dB / RX50dB、幅度0.2、rate2.5MS/s、TX BW500kHz / RX BW1MHz，
+100kHz偏移意味着名义空口单音434.020MHz。它是后续有限计划的参考，不是所有波形的
通用功率设置或连续发射安排；调制波形/新带宽等仍按实际任务登记，不自行恢复模型。
本次仅记录偏好，不修改生产扫描范围、冻结profile或历史runner默认，不启动RF。

后续围绕当前接法界定一个有限非模型链路单元；普通“继续”沿用频段选择，不重做已完成试验。

实验资料按需打开，不默认展开正文或证据：

- [当前433MHz接法的验证依据](validation/B210_433920_ANTENNA_VALIDATION_2026-09-10.md)
...
已授权的这批独立试验结束后，不自行增加巡航或无预算采集。模型工作仍暂停，
无可自动启动的校准、locked test或A1；恢复主线须用户明确选择并满足下列独立证据条件。
...
- NX+B210仅在明确授权和登记计划内作有限外部信号源；后续实验频段按上方433.920MHz选择执行。P201/生产Agent保持RX-only。
