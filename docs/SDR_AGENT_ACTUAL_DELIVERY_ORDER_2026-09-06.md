...
AGX B210 RF A TX/RX经30dB衰减器及15cm同轴接P201 RX1，覆盖此前天线接法。
...
默认值是保守起点，不代表有线质量通过。已有低增益失败不重复；当前30dB接法的有限80/40结果见下文。
...
当前保留30dB；收发与停止通过，条件SINR余量仍未达标，残差集中在TX LO偏移对应的+250kHz附近。
