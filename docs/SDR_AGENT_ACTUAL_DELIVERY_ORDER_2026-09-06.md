...
主机迁移后的新plan默认AGX USB、TX0/RX20，可显式登记TX0/70/80和RX20/40/50；
默认值是后续低增益有线检查起点，有线尚未实测；本轮AGX天线实测显式使用80/40，见下文。
...
不重复这个已完成小批。当前接法是天线，有线50dB串联尚未确认；
后续受控基线仍应先确认接法、登记TX0/RX20短时有线计划，检查功率/削顶再逐步调整，
不直接沿用空口80/40。扩大天线实验需新有限计划，保留UHD连续性与质量边界。
