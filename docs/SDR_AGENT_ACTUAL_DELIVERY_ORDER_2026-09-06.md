...
更新：2026-09-07。本文只维护施工顺序，不复制完成状态。
...
进入已安装制品，Worker、识别 Web、profile/校准和准入配置未整体部署；这不是 A1。

| 接下来 | 独立单元 | 完成边界 |
| --- | --- | --- |
| 1 | Web 后台升级与会话恢复验收 | 先审计旧安装与当前源码，复用现有实现；验证重启不重复扫描、request/session/generation 隔离、迟到结果拒绝、停止/断连恢复、结果保留/查看/删除和可回滚安装；不开放识别 |
| 2 | 非模型运维配置部署 | 在 Web/Planner 健康接口兼容后，单独安装和验证已有 O1a 日志轮转、只读健康与本地去重告警；用户结果不参与日志或升级清理 |

这两个单元各自测试、清理、提交并立即 push，不合为一次大升级。旧 Web 与统一
CLI 的普通 RX 兼容核验已经通过；下一单元验证的是新版 Web 的实际升级边界。
当前源码存在识别接口不构成部署 Worker、训练或放开模型能力的授权。

## 识别主线：按证据条件恢复

| 顺序 | 单元 | 前置条件与边界 |
| --- | --- | --- |
| 1 | V1b / V3a | 获得并审核独立 known-RF/OOD 标签及覆盖；解决正式名称映射依据。工具可用不代表数据已经齐备 |
...
- FPGA/BOOT、P201 TX 和 NX 推理 offload 不进入主线；第7章辐射源身份识别留作后续独立范围。
