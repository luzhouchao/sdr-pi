...
S6b 完成源码及隔离实机浏览器验收；2026-09-07 的[独立 Web 升级](../validation/WEB_RECOVERY_UPGRADE_VALIDATION_2026-09-07.md)
已安装当前 Web 制品，工程识别未配置，生产识别仍不可用。
...
Web stdout/stderr 与退出通知同时绑定 session ID 和子进程实例；会话重建后拒绝
旧实例输出，已失效的排队 actor 不启动或领取首次扫描。切换中的 running 扫描
标记失败，恢复不补扫；输出流结束后才释放单子进程所有权。
