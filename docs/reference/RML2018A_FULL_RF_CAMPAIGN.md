...
缓冲块须等写盘和GPU消费者都用完才回收，不能覆盖仍在使用的IQ。
...
主动停止、设备异常、丢样或持续积压时有限停止并记录；内存积压可由已落盘原始数据承接，
队列/空间不足时必须有有限停止和失败记录，
不能静默丢样或让识别模型提前运行。先RX就绪再TX的握手必须来自真实接收状态，不能用固定sleep冒充。

**实现边界**：目前完成的是[实验GPU payload后端](../../jetson-agx/sdrharness/scripts/rml2018a_campaign_gpu.py)
及1024条同Z行映射、[离线实测入口](../../jetson-agx/sdrharness/scripts/benchmark-rml2018a-gpu.py)，
以及下述SigMF/HDF5存储接口；尚未连接真正的内存双消费者/连续收发队列。
GPU支持批量RMS归一化和条件SINR交叉拟合；FP64计算/FP32输出，门限和失败理由保持。
尚未接入连续收发路径，尚未GPU化导频同步/保护区校正，也没有开始整档或全库采集。
24/1024/4096/8192条实测见[验证](../validation/RML2018A_FULL_RF_CAMPAIGN_2026-09-10.md#2026-09-13gpu批量payload预处理与1024条分块)。

当前Controller sweep使用`CAPTURE_IQ_INLINE`，每次最多256KiB sc16；sdrd先收完并写本地临时文件，
再base64返回。`1048576`采样上限属于`CAPTURE_SUMMARY`功率统计接口，不能作为IQ传输上限引用。
旧TX helper每24条构造一次USRP、重复同一帧约4秒；现有同步搜索也只支持65535点的小窗。
要完成目标，下一单元需在同一Controller/sdrd中实现有界持续RX、分段传输和就绪通知，
配合B210常驻有限TX及块标记/漂移跟踪；禁止仅提高常量或轮询旧小窗后宣称无缝连续。
上述新接口需验证真实收发/写盘重叠、序号与丢样、取消/断连及射频完整恢复，之后才能安排全库计划。
