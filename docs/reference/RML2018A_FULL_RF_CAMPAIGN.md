...
| 命令 | 作用 |
| --- | --- |
| `ledger-plan --root <新总根> --ledger-max-new-batches N` | 核对完整源SHA并冻结新增批次/TX、存储和失败池额度；默认N=2，不发射 |
...
本轮总计划只允许两批新采集，已达到登记上限；不得改写其计划扩额。
新的运行范围须新总计划并导入符合条件的完成块。当前只导入完整event-chunk；
正式全库还需明确其他历史诊断成员的标准化重放/血缘及完整覆盖规则，不能只绕开旧源行后宣称覆盖全部数据。
实测、恢复、边界及精确清理见
