...
aggregate results. The Raspberry Pi deployment remains a rollback baseline. The installed unified
`sdr-agent` supplies interactive, scripted and strictly read-only health modes;
Web launches the same CLI. The Web HTTP server remains a separate service.
...
The daemon now also supports RX2 through startup configuration, but the frozen
Controller/profile contract continues to require RX1. See the
[port validation](../validation/P201_RX_PORT_SELECTION_VALIDATION_2026-09-07.md).
...
displays stored traces and provides an indexed manual-delete path. S6a/S6b recognition-record persistence, per-record deletion and the browser
loop are implemented and isolated-validated; the installed older Web does not
yet expose that production recognition deployment. See the
[archive contract](RECOGNITION_ARCHIVE_S6A_INTERFACE.md).
...
through the standalone engines and S5/S6b engineering Runner/Web loop, including
private spool cleanup, joint cancellation, correlated feedback and radio
restoration. Production admission still lacks independent RF/OOD evidence and
frozen calibration/rejection references, so `recognizer_available=false`.
The S1 capability checks, S2 full result/compact observation and S3 lifecycle
are implemented; the unified CLI contains Controller-side code, while the
production Worker/recognition Web/admission configuration is not deployed.
IQ, tensors and full logits remain outside Planner JSON.
...
queue/cancellation and shared-GPU resource validation has since completed in
S3/S4a/S4b; GPU temperature remains unmeasured under the operator's explicit
exception. The production deployment remains a separate gate. See the
[GPU lease](GPU_LEASE_S4A_INTERFACE.md) and [resource contract](GPU_RESOURCE_S4B_INTERFACE.md).
...
- The installed Planner lease covers Planner inference. S4a implements and
  isolated-validates the shared Spark/Mamba GPU gate; production recognition
  must deploy that gate before Mamba capability can become available. This gate protects cancellation, late-result and
