...
- [ ] 1024条块连续收发/GPU处理与双份语料保留：同一Controller/sdrd真实RX就绪后启动B210有限常驻TX；同时RX/分段传输/原始IQ落盘/GPU处理后另存，验收丢样/积压/取消/恢复。26档全收全处理落盘后才加载模型；Spark保持停止，原始及处理数据保留，当前GPU后端未接入该路径。
