...
- [ ] 有线波形质量：高源SNR已有八类RX40及其余16类RX50有限覆盖；旧三批保护区异常本轮未重现但来源未确定，AM估计/模型限制、统一设置新样本与低源SNR仍未解决。局部有限验证不完成稳定全库质量、物理SINR校准或生产准入。
...
- [ ] 多类校正稳定性与低源SNR覆盖：其余16类及四类新源行单元完成，32PSK/32QAM/FM旧拒绝在本轮未重现但物理来源仍未确定；统一RX50的24类新样本确认、低源SNR分层及AM估计适用性尚未完成。保留LO新1条回退、时序净回退与AM源模型限制，不以少量通过宣布全库质量完成。
