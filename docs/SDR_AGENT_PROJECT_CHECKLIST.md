...
- [x] 完整验证集顺序执行入口：4项隔离测试、启动阶段STOP/恢复及首批8192行实收封存通过；后台单实例已启动，数据集完成屏障生效。运行留存/清理边界见[启动证据](evidence/AMC_VALIDATION_CAMPAIGN_2026-09-27.json)。
...
- [x] 三数据集source/store与离线前置：原始源/seed42划分、352/320/832固定成员、Hisar稀疏标签映射、raw/guard原生存储及29项回归通过；24当前模型CUDA数值门通过。原生频谱32/2/555窗失败；2018A两档各768窗均通过。固定4倍RRC理想往返最大误差0.05194%，只作候选。清理及原件保留见[审计](evidence/AMC_SOURCE_STORE_2026-09-26.json)和[边界](reference/RML2018A_RF_REPRODUCTION.md#69-源数据存储与频谱前置检查2026-09-26)。
- [x] 四套RRC候选离线同步/相消与存储接入：用户同意2018A另加768窗先导，原基线保留；2272真实源窗的带CFO/LO/噪声/量化软件链路全部同步，guard及ADC区间通过，最大误差0.8631%。修复长帧频率峰歧义/导频CFO偏置；48项回归通过、1 CUDA跳过，重复输出一致。见[审计](evidence/AMC_RRC_CHAIN_2026-09-26.json)和[方法边界](reference/RML2018A_RF_REPRODUCTION.md#611-四套rrc候选的离线同步与相消接入2026-09-26)。未RF/模型/部署。
- [ ] 四套候选先导实机接入：接收/持久化工作线程、停止/丢包验证、条件SINR合同及当前硬件基线/制品/有限计划尚未完成；旧2048行入口不能直接运行。按用户最新统一profile选择覆盖四套；不能由离线测试勾选实机验收。
...
- [ ] 新三套D10端到端：RML2016A/2016B/HisarMod2019均seed42；权重/原生CUDA门通过，源数据/划分已接入；有限收发、raw/guard识别和接收SINR报告尚未完整交付。
...
- [x] 新seed统一随机滚降/时间偏移四组：3200 D10预测、700416已知符号零错；BPSK2点稳定64/64、QPSK8点62～64/64，16QAM/AM仍失败。2测试、50数值门探针、3200输入哈希及50组/48配对读回通过；删375缓存25,639,580字节，留8文件1,343,252字节，无RF/训练/部署。见[完整结果](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#新种子随机滚降与时间偏移对照2026-09-26)。
- [x] 自生成波形物理核验及论文依据：768数字窗175104符号零错、最差EVM0.49534%，128 AM窗消息/边带/包络通过；2018引用论文证实随机滚降/时间偏移等，但本地每类sps和AM参数仍未知。无模型/RF/训练，删3文件1,789,402字节，留9文件274,525字节；见[检查及边界](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#自生成波形物理核验与生成依据2026-09-26)。
- [x] 自生成信号输入约定与符号率/相位对照：1792新预测，sps2 BPSK64/64而sps8为0；QPSK sps8为63/64、其他档0，16QAM/AM仍失败。28数值探针、320父logits逐值复现、输入哈希和28组独立统计通过；删375缓存25,592,442字节，留7文件484,115字节。无RF/训练/部署，见[参数敏感性与限制](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#自生成信号符号率与相位对照2026-09-26)。
- [x] 用户授权自生成1024点离线对照：五类各64波形、3840配对观察及20,800 D10预测完成；2测试、七路数值门、输入重生成及13组读回通过。干净QPSK63/64其余四类0；oracle有319误差下降但识别回退，低SINR总体却改善，未确立实测根因。删376缓存25,821,384字节，留11文件3,738,513字节，无RF/训练/部署；见[完整结果及限制](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#自生成1024点可控相消对照2026-09-26)。
- [x] 新会话交错试验的源行映射与离线排程：存储支持显式观察顺序及重复源行，失败窗口保留映射，拒绝冒充完整SNR；672个seed42 validation源波形×三档、另32尾控制，真实Y/Z核对及30项测试通过。保留1文件81,428字节，无IQ副本，临时目录已清；见[准备记录](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#新会话交错试验映射准备2026-09-26)。
- [ ] 固定RMS规则的新会话交错RF验证：排程尚需接入有界收发入口，完成新鲜基线/背景及执行计划；新RF、恢复后D10推理与条件SINR对比未执行。不能用离线帧检查替代实机验证。
- [x] D10接收指标规则探索及固定大样本复核：7/6采集分组，RMS规则主172218剩余成员49.11%→50.70%，但4/6采集退化、低SINR仍不及raw；标量动作/SINR核验通过，不部署。留19文件5,973,464字节（含NumPy兼容失败）、删1缓存123,432字节；见[结果](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#接收端指标选择相消强度2026-09-26)。
- [x] D10固定25%剩余共同成员检验：367001主成员/2496控制分列，总体53.92%低于完全相消55.10%，低SINR22.81%仍低于raw23.39%；旧成员数值/top1门、39组独立统计通过。留16文件58,681,330字节、删376缓存25,748,886字节；见[结果](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#固定25相消的剩余共同成员验证2026-09-26)。
- [x] D10五档部分相消/保护区关联：2496同成员独立RMS，raw/guard逐值重现，三候选输入SHA及65组统计独立核验；低SINR正确276/272/233/215/207，25%未优于不相消。留17文件1,663,640字节、删376缓存25,711,688字节；见[结果](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#d10部分相消与保护区关联2026-09-26)。
- [x] D10补齐369497共同验证成员raw/guard：738994预测完成，旧2496两路logits逐值一致；88配对/176自身SINR分层独立核验，8模型3图完成。留19文件87,923,648字节、删377缓存25,836,886字节，无RF/训练/部署；见[大样本结果](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#d10补齐369497共同验证成员2026-09-22)。
...
- [x] D10固定尺度窄带回添：同2496成员/原val；主576不添/半幅/完整/+90°/+180°/125k/375k正确105/137/190/189/189/27/181。原分量精确重现raw，八路数值及独立哈希/能量/谱峰/统计复核通过；留14文件2,286,332字节、删376缓存25,713,620字节。无RF/训练/部署，不作训练捷径归因，见[结果与边界](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#d10固定尺度窄带回添对照2026-09-22)。
- [x] D10相消/RMS四路分离干预：同2496输入、A/D历史logits逐值一致；主576原raw/只相消/只放大/当前guard正确190/105/176/131，原98回退仍93在无重归一化时错。四路数值门及独立输入/统计复核通过，留15文件1,259,172字节、删376缓存25,873,563字节，无RF/训练/部署，见[干预与边界](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#d10相消与rms尺度分离干预2026-09-22)。
- [x] D10低源SNR载荷特征排查：2496原输入SHA一致、576主成员全保留；98回退频谱/源相干度均改善，中心功率变化最大5.417e−7、去除能量至少99.99954%在LO带。20格仅35回退可匹配，未建立因果；验证与保留/失败边界见[记录](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#d10低源snr载荷特征排查2026-09-22)。无新推理/RF/训练/部署。
- [x] D10替换D8同2496成员离线对照：源码哈希、126958参数严格加载、原val重建、四路49探针单条/批量数值门及独立汇总通过；总raw/guard1286/1364、低源SNR190/131，98回退/39纠正，低档退化仍存在。留12文件1,235,912字节，删376缓存文件25,711,366字节，无RF/训练/部署。见[结果与证据边界](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#d10同成员重跑完成2026-09-22)。
- [x] 低源SNR相消回退逐行关联：同2496原val，主576条106回退/36纠正；106回退条件SINR均提高，78源域正确。AM-DSB-WC18/FM10回退；同类同Z仅14格25回退可比，未建立相位/残余稳定关联。4测试、2496行/8指标独立复核通过；首次RSS超1GiB失败保留，单文件缓存修正后原门通过；删4文件1,196,676字节、留20文件2,827,409字节，无RF/推理/训练/部署。见[关联与边界](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#低源snr相消回退的逐行关联2026-09-17)。
- [x] 固定频率保护区LO幅相诊断：同2496原val/ADC，14976历史半区幅度/SE重现及2496独立复算通过；相位曲率双半区相关0.978，888帧共同通过四块长描述性4SE筛查，幅度极差中位约0.85%。支持相干相位时变结构，非校准显著性/器件归因；6合成测试及图表通过，保留13文件11,280,182字节、删1缓存123,432字节，无RF/推理/训练/生产变更。见[诊断与边界](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#固定频率保护区lo幅相时序诊断2026-09-17)。
- [x] 增加前置保护区的相消单因素对照：同2496原val/ADC/同步/D8/RMS，基线输入/logits容差及top-1一致100%通过；候选2379应用/117原样保留，总正确1341→1331（纠正4/回退14），AM154不变，共同留出误差中位+2.43%，不启用。6合成测试/676组独立复核通过，保留24文件28,500,734字节，清理354文件24,465,207字节，无新RF/训练/生产改动。见[失败候选与下一诊断](validation/RML2018A_OFFLINE_BASELINE_2026-09-16.md#增加前置保护区的单因素相消对照2026-09-16)。
