...
- [ ] 26档完整实机收发与双份语料：后台脚本先运行联合停止/恢复、完整RX后的缓存重放和跨进程完成跳过验证，通过后自动采集剩余档；以运行根的验证记录及acquisition-complete.json为实际完成依据。Spark保持停止，全部原始/处理语料保留，未运行模型；真实断电恢复和全库模型识别未验证，不把后台启动当采集完成。
