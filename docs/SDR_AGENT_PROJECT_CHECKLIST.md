...
- [ ] 26档封存语料冻结模型识别：已补齐HDF5只读接口、输入/来源校验、常驻batch1推理、逐块原子logits与源Z/类别/SINR统计；5项针对性测试及三档真实块读取校验通过。用户已授权后台执行，完整2496块结果和退出清理以推理根complete/retention为准，不把启动等同全量识别完成。
