...
2. 确认只有一个接收所有者和一个预期 sdrd，不启动第二个 collector。P201 是
   ...
   用户2026-09-13最新确认P201 RX1独立接50Ω负载，当前按RX-only负载对照，B210保持停发。
   此前AGX USB B210 RF A TX/RX经20dB衰减器及同轴接P201 RX1是回放参考接法；
   恢复收发前须确认用户已重新接回该链路。改变衰减前须停发并按新接法登记有限计划，不自行切到其他频段。
