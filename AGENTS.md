...
用户随后暂停旧全量任务，再明确授权使用4090原seed42验证集运行8个seed42模型；
用户随后明确从零重跑并删除前两轮结果；旧全量/复用验证集结果根已删除，
新验证集任务使用fresh结果根，禁止导入旧预测，不恢复旧全量范围。
原70/15/15索引文件的val为383,385源行；raw/guard还须与usable/strict_quality_pass相交，
保存validation_rank及共同源行对照，完成后绘制24张混淆矩阵。
...
- 用户2026-09-22进一步授权D10在既有369497共同通过质量检查的seed42验证成员上补跑raw/guard工程推理，并更新SINR对比；不追加source、不训练、不重采RF、不切换生产服务。
- 用户2026-09-22明确D10取代D8作为当前对比模型：新实测图表和汇总不再列D8，保持raw/guard，不追加source曲线。D10只使用实际已完成的预测范围，不能挪用D8成绩；旧封存证据按第6节保留。
...
用户2026-09-27明确授权按已验证统一RRC短帧方案推进四套完整seed42验证集收发：2016A 33000、2016B 180000、2018A 383385、Hisar 117000行，包含各源Z档。顺序为2016A→2016B→2018A→Hisar；上一数据集全部接收、raw IQ及raw/guard落盘并校验封存后才可启动下一套。允许每套内部有界分批和断点续跑，不跳过失败批次、不混入train/test、不复用旧预测。此最新授权覆盖此前仅高Z有限先导的范围限制，仍不授权训练或生产识别切换。
