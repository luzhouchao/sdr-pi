#!/usr/bin/env python3
"""D10 total-input-power matched tone/broadband controls; no RF or training."""
import argparse
import hashlib
import json
import os
...
NOISE_SEEDS=(20260922,20260923,20260924)


def match_power(x,target):
    power=np.mean(abs(x)**2,axis=1)
    ev.require(np.isfinite(power).all() and (power>0).all(),'nonzero finite power')
    out=x*np.sqrt(target/power)[:,None]
...
def summarize(truth, logits, mask):
    ...
    result=dict(rows=int(mask.sum()),correct={k:int((v&mask).sum()) for k,v in good.items()},pairs={})
    for name in ('tone_power','noise0','noise1','noise2'):
        a=good['scale_only'];b=good[name]
        result['pairs'][name]=dict(corrected=int((~a&b&mask).sum()),regressed=int((a&~b&mask).sum()))
    reg=good['raw']&~good['guard']&mask
    result['original_regressions']=int(reg.sum())
    result['rescued_original_regressions']={k:int((v&reg).sum()) for k,v in good.items()}
    return result
...
def main(out):
    ...
    plan=dict(schema='d10-power-match-v1',identities=identities,split=split,source_ids_sha256=ah(ids),rows=2496,source_commit=manifest['commit'],
              ...
              planes=['raw','guard','scale_only','tone_power','noise0','noise1','noise2'],design='b=G/r,d=raw-b; scale_only=power_match(b),tone_power=raw,noise_i=power_match(b+white_noise_i); match each added white component energy to d before final normalization; target per-row original raw power; noise seeds20260922/23/24 each PCG64 SeedSequence([seed,source_row]); retain all three, no winner selection; original2496 primary sourceZ -10..0',noise_seeds=list(NOISE_SEEDS),power_rtol=2e-7,no_training=True,no_rf=True,production_changes=False)
    ...
    raw=data['raw'][:,0].astype(float)+1j*data['raw'][:,1].astype(float)
    d=raw-b; target=np.mean(abs(raw)**2,axis=1); added_power=np.mean(abs(d)**2,axis=1)
    data['scale_only']=match_power(b,target)
    data['tone_power']=data['raw'].copy()
    noise_audit={}
    for j,seed in enumerate(NOISE_SEEDS):
        noise=[]
        for row in ids:
            rng=np.random.Generator(np.random.PCG64(np.random.SeedSequence([seed,int(row)])))
            v=rng.standard_normal((2,1024));noise.append(v[0]+1j*v[1])
        noise=np.asarray(noise);noise*=np.sqrt(added_power/np.mean(abs(noise)**2,axis=1))[:,None]
        ev.require(np.allclose(np.mean(abs(noise)**2,axis=1),added_power,rtol=1e-12,atol=1e-15),'added component energy')
        data['noise'+str(j)]=match_power(b+noise,target)
        noise_audit['noise'+str(j)]=dict(seed=seed,noise_complex128_sha256=ah(noise),pre_normalization_power_ratio_percentiles=np.percentile(np.mean(abs(b+noise)**2,axis=1)/target,[0,50,100]).tolist())
    for name in ('scale_only','tone_power','noise0','noise1','noise2'):
        power=np.mean(np.sum(data[name].astype(float)**2,axis=1),axis=1)
        err=float(np.max(abs(power/target-1)))
        ev.require(err<=2e-7,'matched total power')
        input_audit[name]=dict(candidate_sha256=ah(data[name]),power_max_relative_error=err)
    ev.atomic(out/'noise.json',noise_audit)
    ...
    ev.require(np.array_equal(logits['tone_power'],logits['raw']),'tone positive control')
