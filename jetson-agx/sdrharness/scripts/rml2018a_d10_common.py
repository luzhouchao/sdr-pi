#!/usr/bin/env python3
"""D10 inference on the sealed common validation population, raw/guard only."""
...
REPLAY=Path('/var/tmp/sdrharness-dev/rml2018a-d10-replay-20260922')
...
def main():
    ...
    for audit,name in [('RML2018A_SINR_STRATIFICATION_2026-09-22.json',PARENT/'joined-predictions.npz'),('RML2018A_D10_REPLAY_2026-09-22.json',REPLAY/'predictions.npz')]:
    ...
    for plane in ('raw','guard'):
        rec=next(r for r in inputs if '_'+plane+'_' in r['path']);ev.unchanged(rec)
        with h5py.File(rec['path'],'r') as f:
            ...
            def read(take):
                r=rows[take];ix=np.argsort(r);x=np.ascontiguousarray(f['iq'][r[ix]][np.argsort(ix)],dtype=np.float32)
                ev.require(x.shape==(len(r),2,1024) and np.isfinite(x).all(),'input shape/finiteness')
                rms=np.sqrt(np.mean(np.sum(x.astype(np.float64)**2,axis=1),axis=1));ev.require(np.max(abs(rms-1))<=2e-6,'RMS')
                return x
            ...
            logits=np.empty((n,24),np.float32);io_order=np.argsort(rows);h=hashlib.sha256();last=time.monotonic()
            for start_row in range(0,n,128):
                ...
                logits[take]=ev.predict(model,x,torch,'fp32')
                ev.require(not (out/'STOP').exists(),'STOP requested')
                if time.monotonic()-last>30 or start_row+128>=n:
                    done=min(start_row+128,n);ev.atomic(out/'progress.json',dict(plane=plane,done=done,total=n,elapsed_seconds=time.monotonic()-start));print(plane,done,n,round(time.monotonic()-start,1),flush=True);last=time.monotonic()
            ref=prior[plane+'_logits'];actual=logits[sub]
            ...
            checks[plane]=dict(rows=n,prior_rows=len(sub),prior_max_abs=float(abs(actual-ref).max()),singleton_max_abs=float(abs(bat-single).max()),selected_input_stream_sha256=h.hexdigest(),correct=int((logits.argmax(1)==y).sum()))
            np.savez(out/(plane+'-predictions.npz'),source_row=ids,validation_rank=member['validation_rank'],truth=y,logits=logits,predicted_class=logits.argmax(1))
            all_logits[plane]=logits;ev.atomic(out/'verification.json',checks)
        ev.unchanged(rec)
