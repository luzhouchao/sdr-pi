...
def main(root, output):
    ...
    assert audit['status'] == 'completed' and len(audit['sweeps']) == 18
    surveys = audit['sweeps'][:3]
    ...
    for i, s in enumerate(surveys):
        ...
        ax[0,0].plot(centers, [v['raw_rms']['p95'] for v in stats], color=colors[i], label=f'Round {i+1}', lw=1.2)
        ax[0,1].plot(centers, [v['raw_rms']['p50'] for v in stats], color=colors[i], label=f'Round {i+1}', lw=1.2)
    for panel, title in zip(ax[0], ['High-activity tail: 128-sample RMS p95', 'Typical background: 128-sample RMS median']):
        panel.set(xlabel='RX center frequency (MHz)', ylabel='ADC RMS (log scale)', yscale='log', xlim=(2400,2483.5), title=title)
    ...
    im = ax[1,0].imshow(matrix, aspect='auto', extent=(2400,2483.5,3.5,.5), cmap='magma', vmin=0)
    ...
    for i in range(3):
        rows = [s['rows'][0] for s in audit['sweeps'][3:] if s['round']==i]
    ...
    fig.suptitle('2.4 GHz antenna background | RX gain 20 dB | 31.2 ms per point\nFrequency points and rounds are sequential; no protocol/source labels', fontsize=13)
    ...
    for i,c in enumerate(focus_centers):
        for j in range(3):
            s = next(s for s in audit['sweeps'][3:] if s['round']==j and s['rows'][0]['center_hz']==c)
