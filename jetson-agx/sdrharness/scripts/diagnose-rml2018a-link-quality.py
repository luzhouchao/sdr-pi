...
def capture(gain):
    ...
    plans=[dict(sweep_id=f'link-background-{generation+i}',session_generation=generation+i,
        ...
        aggregate_frames=1,point_timeout_ms=1000,detection_threshold_db=12.) for i in range(3)]
    paths=[f'/tmp/sdr-agent-dev/agx-sweep-{p["session_generation"]}-0' for p in plans]
    free=shutil.disk_usage(root.parent).free;c.require(free>3*c.RX_SAMPLES*4+32*1024**2,'disk budget')
    root.mkdir(mode=0o700)
    audit=dict(schema='rml-link-background-v1',script_sha256=c.file_hash(__file__),plans=plans,
        root=str(root),maximum_rx_bytes=3*c.RX_SAMPLES*4,free_bytes_before=free,
        expected_wall_seconds=30,deadline_seconds=120,p201_paths=paths,nx_paths=[],
    ...
    try:
        for i,plan in enumerate(plans):
            ...
            if gain==50:_,seal=r.native_check(dest,plan)
            else:
                original,_=bg.bg.native(dest,plan);report=r.document(dest/'report.json')
            ...
            audit['rows'].append(dict(point=i,seal=seal))
...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('command',choices=['capture-background','analyze'])
