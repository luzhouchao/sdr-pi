...
TAP_BASE = 0x43C00000
SELFTEST_BASE = 0x43C30000
BOOT_HASH_PLACEHOLDER = "TO_BE_FILLED_AFTER_BOOTGEN"
FILES = ["BOOT.bin", "devicetree.dtb", "uEnv.txt", "uImage", "uramdisk.image.gz"]

EXPECTED_HASHES = {
    "BOOT.bin": BOOT_HASH_PLACEHOLDER,
...
EXPECTED_V10S5_REGS_BEFORE_START = {
    "version": (0x00, 0x53355430),
    "capability": (0x18, 0x0000003F),
    "build_id": (0x1C, 0x56313035),
    "abi_version": (0x20, 0x000A5000),
    "expect_done_mask": (0xE0, 0x0000003F),
    "expect_error": (0xE4, 0x00000000),
    "test_capability": (0xF4, 0x0000003F),
    "test_build_id": (0xF8, 0x56313035),
    "test_abi_version": (0xFC, 0x000A5000),
}

EXPECTED_V10S5_REGS_AFTER_START = {
    "done_mask": (0x0C, 0x0000003F),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate V10S5 combined self-test after physical SDR power-cycle.")
    ...
    parser.add_argument("--boot-sha256", default="", help="Expected V10S5 BOOT.bin sha256; overrides the placeholder.")
    args = parser.parse_args()

    expected_hashes = dict(EXPECTED_HASHES)
    if args.boot_sha256:
        expected_hashes["BOOT.bin"] = args.boot_sha256.lower()
    boot_hash_placeholder_used = expected_hashes["BOOT.bin"] == BOOT_HASH_PLACEHOLDER

    result: dict = {
        "timestamp_sec": time.time(),
        "operation": "validate_v10s5_combined_selftest_after_powercycle_via_nx",
        "expected_hashes": expected_hashes,
        "boot_hash_placeholder_used": boot_hash_placeholder_used,
    ...
    try:
        ...
        try:
            ...
            result["sd_companion_hashes_ok"] = all(
                result["sd_hashes"].get(name) == expected_hashes[name]
                for name in FILES
                if name != "BOOT.bin"
            )
            result["sd_hashes_ok"] = (
                not boot_hash_placeholder_used
            ...
            result["v10s5_regs_before_start"] = read_expected_regs(
                sdr, SELFTEST_BASE, EXPECTED_V10S5_REGS_BEFORE_START
            )
            run_id_before = devmem_read(sdr, SELFTEST_BASE + 0x14)
            result["v10s5_run_id_before"] = run_id_before
            ...
            result["v10s5_status_after_start_hex"] = f"0x{status:08X}"
            result["v10s5_status_poll_count"] = polls
            result["v10s5_done_bit"] = bool(status & 0x00000004)
            result["v10s5_fail_bit"] = bool(status & 0x00000008)
            result["v10s5_run_id_after"] = run_id_after
            result["v10s5_run_id_increment_ok"] = run_id_after == ((run_id_before + 1) & 0xFFFFFFFF)
            result["v10s5_regs_after_start"] = read_expected_regs(
                sdr, SELFTEST_BASE, EXPECTED_V10S5_REGS_AFTER_START
            )
        finally:
            sdr.close()
    finally:
    ...
    pre_ok = all(item["passed"] for item in result["v10s5_regs_before_start"].values())
    post_ok = all(item["passed"] for item in result["v10s5_regs_after_start"].values())
    result["passed"] = bool(
        result["sd_hashes_ok"]
        ...
        and v8d0_ok
        and pre_ok
        and result["v10s5_done_bit"]
        and not result["v10s5_fail_bit"]
        and result["v10s5_run_id_increment_ok"]
        and post_ok
    )
