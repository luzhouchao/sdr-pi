...
TAP_BASE = 0x43C00000
SUM6_VERSION = 0x53554D36
SUM6_ABI_VERSION = 0x00010000
SUM6_CAPABILITY = 0x000000FF
SUM6_MAX_CORR_FRAME = 0x0000FFFF
SUM6_BUILD_ID = 0x56360001
...
def signed64_from_words(lo: int, mid: int, hi: int = 0) -> int:
    ...
    sign_ext_ok = (hi == 0xFFFFFFFF if value & (1 << 63) else hi == 0)
    signed = value - (1 << 64) if value & (1 << 63) else value
    return signed if sign_ext_ok else signed


def decode_summary_flags(value: int) -> dict[str, bool]:
    return {
        "enable": bool(value & (1 << 0)),
        "result_valid": bool(value & (1 << 1)),
        "busy_or_pending": bool(value & (1 << 2)),
        "frame_len_limited": bool(value & (1 << 3)),
        "arithmetic_overflow": bool(value & (1 << 4)),
        "corrected_valid": bool(value & (1 << 5)),
        "snapshot_disabled": bool(value & (1 << 6)),
    }


def decode_limit_flags(value: int) -> dict[str, bool]:
    return {
        "frame_len_limited": bool(value & (1 << 0)),
        "arithmetic_overflow": bool(value & (1 << 1)),
        "corrected_valid": bool(value & (1 << 2)),
        "snapshot_disabled": bool(value & (1 << 3)),
    }


def decode_capability(value: int) -> dict[str, bool]:
    return {
        "dual_rx": bool(value & (1 << 0)),
        "raw_power": bool(value & (1 << 1)),
        "peaks": bool(value & (1 << 2)),
        "cross": bool(value & (1 << 3)),
        "iq_sums": bool(value & (1 << 4)),
        "corrected_numerators": bool(value & (1 << 5)),
        "snapshot_disabled": bool(value & (1 << 6)),
        "wide_corrected_path": bool(value & (1 << 7)),
    }
...
def parse_frame_lens(text: str) -> list[int]:
    ...
    for part in text.split(","):
        ...
        if not part:
            continue
        value = int(part, 0)
        if value < 1 or value > SUM6_MAX_CORR_FRAME:
            raise ValueError("SUM6 frame_len must be in 1..65535")
        values.append(value)
    return values or [64]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Read Summary V6 block ABI registers without touching the active NX SDR chain.")
    ...
    parser.add_argument("--out-json", default="logs/summary_v6_block_abi.json")
    ...
    def capture_once(frame_len: int, iteration: int) -> dict:
        ...
        summary = {
            ...
            "rx0_corr_power_num": signed64_from_words(r(0xBC), r(0xC0), r(0xC4)),
            "rx1_corr_power_num": signed64_from_words(r(0xC8), r(0xCC), r(0xD0)),
            "corr_cross_re_num": signed64_from_words(r(0xD4), r(0xD8), r(0xDC)),
            "corr_cross_im_num": signed64_from_words(r(0xE0), r(0xE4), r(0xE8)),
            "abi_version_hex": f"0x{abi:08X}",
            "capability_hex": f"0x{capability:08X}",
            "capability": decode_capability(capability),
            "limit_flags_hex": f"0x{limit_flags:08X}",
            "limit_flags": decode_limit_flags(limit_flags),
            "max_corr_frame_len": max_corr_frame,
        ...
        corrected_ok = summary["summary_flags"]["corrected_valid"] and summary["limit_flags"]["corrected_valid"] and not summary["summary_flags"]["arithmetic_overflow"]
        checks = {
            "version_is_sum6": version == SUM6_VERSION,
            "abi_version_ok": abi == SUM6_ABI_VERSION,
            "capability_ok": capability == SUM6_CAPABILITY,
            "max_corr_frame_ok": max_corr_frame == SUM6_MAX_CORR_FRAME,
            "build_id_ok": build_id == SUM6_BUILD_ID,
            "snapshot_disabled": int(summary["snapshot_count"]) == 0 and summary["summary_flags"]["snapshot_disabled"],
            ...
            "corrected_valid_without_overflow": corrected_ok,
            "rx0_corr_power_num_match": corrected_ok and int(summary["rx0_corr_power_num"]) == int(derived["rx0_corr_power_num_calc"]),
            "rx1_corr_power_num_match": corrected_ok and int(summary["rx1_corr_power_num"]) == int(derived["rx1_corr_power_num_calc"]),
            "corr_cross_re_num_match": corrected_ok and int(summary["corr_cross_re_num"]) == int(derived["corr_cross_re_num_calc"]),
            "corr_cross_im_num_match": corrected_ok and int(summary["corr_cross_im_num"]) == int(derived["corr_cross_im_num_calc"]),
        }
    ...
    payload = {
        ...
        "operation": "summary_v6_block_abi_no_snapshot",
        ...
        "notes": [
            "SUM6 validates the register-only block ABI and widened corrected path metadata.",
            "It does not start ROS, robot_control, SDR streaming runtime, mapping, RTAB-Map, or motion paths.",
            "Use V5 as the first hardware burn target; use V6 after V5 validates or when testing ABI/capability registers.",
        ],
    }
