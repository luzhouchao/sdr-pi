...
set project_dir [file join $repo_dir vivado_out phase1_fft_shadow_selftest_ooc_project]
...
if {$project_status eq "PASS"} {
    if {[catch {
        create_project -force phase1_fft_shadow_selftest_ooc $project_dir -part xc7z020clg400-2
        foreach f $rtl_files {
            add_files -norecurse $f
        }
        set_property top p201_fft_shadow_selftest_axi_regs [current_fileset]
        ...
        if {[string match "*Complete*" $run_status]} {
            ...
            open_run synth_1 -name phase1_fft_shadow_ooc_synth
...
puts $fh "# Phase 1 FFT Shadow Self-Test OOC Report"
puts $fh ""
...
puts $fh "- Top: `p201_fft_shadow_selftest_axi_regs`"
puts $fh "- Clock target: `8.138 ns`"
puts $fh "- Utilization report: `$util_file`"
puts $fh "- Timing report: `$timing_file`"
puts $fh "- Scope: PC-only isolated register ABI fixture for `fpga_fft_shadow`; no live AD9361 coupling."
puts $fh ""
...
puts "PHASE1_FFT_SHADOW_OOC_REPORT=$report_file"
puts "PHASE1_FFT_SHADOW_OOC_PROJECT_STATUS=$project_status"
puts "PHASE1_FFT_SHADOW_OOC_SYNTH_STATUS=$synth_status"
...
if {$project_status ne "PASS" || $synth_status ne "PASS"} {
    error "Phase 1 FFT shadow self-test OOC failed: $error_text"
}
