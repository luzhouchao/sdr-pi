...
set v10s5_rtl_dir [file join $repo_dir experiments v10s5_combined_selftest rtl]
set reports_dir [file join $repo_dir reports stage_v10s5_combined_selftest]
set project_dir [file join $repo_dir vivado_out v10s5_combined_selftest_ooc_project]
...
if {$project_status eq "PASS"} {
    if {[catch {
        create_project -force p201_v10s5_combined_selftest_ooc $project_dir -part xc7z020clg400-2
        foreach f $rtl_files {
        ...
        if {[string match "*Complete*" $run_status]} {
            ...
            open_run synth_1 -name v10s5_ooc_synth
...
puts $fh "# V10S5 Combined Self-Test OOC Report"
puts $fh ""
...
puts $fh "- Top: `p201_v10s5_combined_selftest_axi_regs`"
puts $fh "- Clock target: `8.138 ns`"
puts $fh "- Utilization report: `$util_file`"
puts $fh "- Timing report: `$timing_file`"
puts $fh "- Scope: isolated AXI-Lite self-test wrapper mirroring the planned V10S0 FFT-family and V10S4 non-FFT lanes."
...
puts "V10S5_OOC_REPORT=$report_file"
puts "V10S5_OOC_PROJECT_STATUS=$project_status"
puts "V10S5_OOC_SYNTH_STATUS=$synth_status"
...
if {$project_status ne "PASS" || $synth_status ne "PASS"} {
    error "V10S5 OOC failed: $error_text"
}
