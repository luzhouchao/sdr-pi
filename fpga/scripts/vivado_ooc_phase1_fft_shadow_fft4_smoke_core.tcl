...
set reports_dir [file join $repo_dir reports stage_phase1_fft_shadow_fft4_smoke_core]
set project_dir [file join $repo_dir vivado_out phase1_fft_shadow_fft4_smoke_core_ooc_project]
...
if {$project_status eq "PASS"} {
    if {[catch {
        create_project -force phase1_fft_shadow_fft4_smoke_core_ooc $project_dir -part xc7z020clg400-2
        foreach f $rtl_files {
            add_files -norecurse $f
        }
        set_property top p201_fft_shadow_fft4_smoke_core [current_fileset]
        ...
        if {[string match "*Complete*" $run_status]} {
            ...
            open_run synth_1 -name phase1_fft_shadow_fft4_smoke_ooc_synth
...
puts $fh "# Phase 1 FFT Shadow FFT4 Smoke Core OOC Report"
puts $fh ""
...
puts $fh "- Top: `p201_fft_shadow_fft4_smoke_core`"
puts $fh "- Clock target: `8.138 ns`"
puts $fh "- Utilization report: `$util_file`"
puts $fh "- Timing report: `$timing_file`"
puts $fh "- Scope: PC-only 4-point complex FFT smoke core with power and peak summary; no live AD9361 coupling."
puts $fh ""
...
puts "PHASE1_FFT_SHADOW_FFT4_OOC_REPORT=$report_file"
puts "PHASE1_FFT_SHADOW_FFT4_OOC_PROJECT_STATUS=$project_status"
puts "PHASE1_FFT_SHADOW_FFT4_OOC_SYNTH_STATUS=$synth_status"
puts "PHASE1_FFT_SHADOW_FFT4_OOC_TIMING_STATUS=$timing_status"
...
if {$project_status ne "PASS" || $synth_status ne "PASS" || $timing_status ne "PASS"} {
    error "Phase 1 FFT shadow FFT4 smoke core OOC failed: $error_text"
}
