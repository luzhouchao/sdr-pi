...
set reports_dir [file join $repo_dir reports stage_v10s123_nonfft_ooc]
set project_dir [file join $repo_dir vivado_out v10s123_nonfft_ooc_project]
...
set rtl_files [list \
    ...
    [file join $repo_dir experiments v10s123_nonfft_ooc rtl p201_v10s123_nonfft_ooc_top.v] \
]
...
if {$project_status eq "PASS"} {
    if {[catch {
        create_project -force p201_v10s123_nonfft_ooc $project_dir -part xc7z020clg400-2
        foreach f $rtl_files {
            add_files -norecurse $f
        }
        set_property top p201_v10s123_nonfft_ooc_top [current_fileset]
        ...
        if {[string match "*Complete*" $run_status]} {
            ...
            open_run synth_1 -name v10s123_nonfft_ooc_synth
...
puts $fh "# V10S1/S2/S3 Non-FFT OOC Report"
puts $fh ""
...
puts $fh "- Top: `p201_v10s123_nonfft_ooc_top`"
puts $fh "- Clock target: `8.138 ns`"
puts $fh "- Utilization report: `$util_file`"
puts $fh "- Timing report: `$timing_file`"
puts $fh "- Scope: PC-only combined OOC check for V10S1 quality stats, V10S2 frame/window, and V10S3 energy/peak reducers."
puts $fh ""
...
puts "V10S123_OOC_REPORT=$report_file"
puts "V10S123_OOC_PROJECT_STATUS=$project_status"
puts "V10S123_OOC_SYNTH_STATUS=$synth_status"
...
if {$project_status ne "PASS" || $synth_status ne "PASS"} {
    error "V10S123 non-FFT OOC failed: $error_text"
}
