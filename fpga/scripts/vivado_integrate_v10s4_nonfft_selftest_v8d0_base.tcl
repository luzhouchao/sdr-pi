...
set reports_dir [file join $repo_dir reports stage_v10s4_nonfft_selftest_integration]
...
foreach old_cell {p201_v10s0_selftest0 p201_v10s4_selftest0} {
    if {[llength [get_bd_cells -quiet /$old_cell]] > 0} {
        delete_bd_objs [get_bd_cells /$old_cell]
    }
}

set cell_name p201_v10s4_selftest0
create_bd_cell -type ip -vlnv p201pro.local:user:p201_v10s4_nonfft_selftest_axi_regs:1.0 $cell_name

set direct_rtl_files [list]
set ip_component [file join $local_ip_repo p201_v10s4_nonfft_selftest_axi_regs_1.0 component.xml]
...
foreach orphan_net [get_bd_nets -quiet p201_v10s4_selftest0_irq*] {
...
if {[llength $addr_seg] > 0} {
    ...
    set_property offset 0x43C20000 $addr_seg
}
...
puts "V10S4_INTEGRATION_REPORT=$report_file"
