...
set reports_dir [file join $repo_dir reports stage_v10s5_combined_selftest_ip]
set project_dir [file join $repo_dir vivado_out v10s5_combined_selftest_ip_packager_project]
set ip_root [file join $vivado_dir ip_repo p201_v10s5_combined_selftest_axi_regs_1.0]
...
foreach f $rtl_files {
    if {![file exists $f]} {
        error "Missing V10S5 combined self-test RTL file: $f"
    }
}

create_project -force p201_v10s5_combined_selftest_ip $project_dir -part xc7z020clg400-2
foreach f $rtl_files {
    add_files -norecurse $f
}
set_property top p201_v10s5_combined_selftest_axi_regs [current_fileset]
...
set_property name p201_v10s5_combined_selftest_axi_regs $core
set_property display_name {P201Pro V10S5 Combined Self-Test AXI Registers} $core
set_property description {Independent AXI-Lite deterministic combined self-test page for V10S0 FFT-adjacent and V10S4 non-FFT SDR FPGA helper modules.} $core
...
set required_packaged_files [list \
    ...
    p201_v10s5_combined_selftest_axi_regs.v \
]
...
if {$packaged_file_check ne "PASS"} {
    error "V10S5 packaged IP source check failed:\n$packaged_file_errors"
}

set integrity_file [file join $reports_dir ipx_check_integrity.txt]
set report_file [file join $reports_dir v10s5_combined_selftest_ip_report.md]
...
write_checkpoint -force [file join $reports_dir p201_v10s5_combined_selftest_synth.dcp]

set fh [open $report_file w]
puts $fh "# V10S5 Combined Self-Test IP Report"
puts $fh ""
...
puts $fh "- Intended BD address: `0x43C30000`"
puts $fh "- Packaged source context: V10S0 FFT-adjacent helpers plus V10S4 quality, frame/window, and energy/peak helpers"
puts $fh "- Note: the top wrapper defaults to internal deterministic mirrors; external sidecar-core integration still needs a separate interface review."
puts $fh ""
puts $fh "No bitstream or BOOT.bin is produced by this script."
close $fh

puts "V10S5_IP_REPORT=$report_file"
puts "V10S5_IP_ROOT=$ip_root"
