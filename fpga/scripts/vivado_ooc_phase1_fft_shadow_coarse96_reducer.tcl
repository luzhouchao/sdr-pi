...
set reports_dir [file join $repo_dir reports stage_phase1_fft_shadow_coarse96_reducer]
set project_dir [file join $repo_dir vivado_out phase1_fft_shadow_coarse96_reducer_ooc_project]
...
if {$project_status eq "PASS"} {
    if {[catch {
        create_project -force phase1_fft_shadow_coarse96_reducer_ooc $project_dir -part xc7z020clg400-2
        foreach f $rtl_files {
            add_files -norecurse $f
        }
        set_property top p201_fft_shadow_coarse96_reducer [current_fileset]
        ...
        if {[string match "*Complete*" $run_status]} {
            ...
            open_run synth_1 -name phase1_fft_shadow_coarse96_ooc_synth
...
puts $fh "# Phase 1 FFT Shadow Coarse96 Reducer OOC Report"
puts $fh ""
...
puts $fh "- Top: `p201_fft_shadow_coarse96_reducer`"
puts $fh "- Clock target: `8.138 ns`"
puts $fh "- Utilization report: `$util_file`"
puts $fh "- Timing report: `$timing_file`"
puts $fh "- Scope: PC-only exact 2048-to-96 coarse PSD max-hold reducer; no live AD9361 coupling."
puts $fh ""
...
puts "PHASE1_FFT_SHADOW_COARSE96_OOC_REPORT=$report_file"
puts "PHASE1_FFT_SHADOW_COARSE96_OOC_PROJECT_STATUS=$project_status"
puts "PHASE1_FFT_SHADOW_COARSE96_OOC_SYNTH_STATUS=$synth_status"
puts "PHASE1_FFT_SHADOW_COARSE96_OOC_TIMING_STATUS=$timing_status"
...
if {$project_status ne "PASS" || $synth_status ne "PASS" || $timing_status ne "PASS"} {
    error "Phase 1 FFT shadow coarse96 reducer OOC failed: $error_text"
}
