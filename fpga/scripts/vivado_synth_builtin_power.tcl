set script_dir [file dirname [file normalize [info script]]]
set repo_dir [file dirname $script_dir]
set hdl_dir [file join $repo_dir hdl_out builtin_power p201pro_builtin_power_hdl_model]
set synth_dir [file join $repo_dir vivado_out builtin_power_synth]
file mkdir $synth_dir

read_verilog [file join $hdl_dir peak_power.v]
read_verilog [file join $hdl_dir p201pro_builtin_power_hdl_model.v]

synth_design -top p201pro_builtin_power_hdl_model -part xc7z020clg400-1

report_utilization -file [file join $synth_dir utilization.txt]
report_timing_summary -file [file join $synth_dir timing_summary.txt]
write_checkpoint -force [file join $synth_dir p201pro_builtin_power_synth.dcp]
