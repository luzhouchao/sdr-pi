...
set hdl_dir [file join $repo_dir experiments v10s0_submodule_selftest rtl]
set vivado_dir [file join $repo_dir vivado]
set reports_dir [file join $repo_dir reports stage_v10s0_submodule_selftest_ip]
set project_dir [file join $repo_dir vivado_out v10s0_submodule_selftest_ip_packager_project]
set ip_root [file join $vivado_dir ip_repo p201_v10s0_submodule_selftest_axi_regs_1.0]
...
set rtl_files [list \
    [file join $hdl_dir p201_v10_fft_bin_power.v] \
    [file join $hdl_dir p201_v10_fft_summary_reducer.v] \
    [file join $hdl_dir p201_v10_fft_stream_summary_top.v] \
    [file join $hdl_dir p201_fft_frame_packer.v] \
    [file join $hdl_dir p201_bandpower_reducer.v] \
    [file join $hdl_dir p201_v10_multilag_corr_module.v] \
    [file join $hdl_dir p201_v10s0_submodule_selftest_axi_regs.v] \
]

create_project -force p201_v10s0_selftest_ip $project_dir -part xc7z020clg400-2
foreach f $rtl_files {
...
set_property name p201_v10s0_submodule_selftest_axi_regs $core
set_property display_name {P201Pro V10S0 Submodule Self-Test AXI Registers} $core
set_property description {Independent AXI-Lite deterministic self-test page for reusable SDR FPGA submodules.} $core
...
# Vivado 2019.1 sometimes drops leaf modules that look like derived helper
# files during IP packaging. Add every reusable RTL source explicitly so BD
# synthesis does not depend on the packager's file classification.
foreach f $rtl_files {
...
if {$packaged_file_check ne "PASS"} {
    error "V10S0 packaged IP source check failed:\n$packaged_file_errors"
}

set integrity_file [file join $reports_dir ipx_check_integrity.txt]
set report_file [file join $reports_dir v10s0_submodule_selftest_ip_report.md]
...
write_checkpoint -force [file join $reports_dir p201_v10s0_submodule_selftest_synth.dcp]

set fh [open $report_file w]
puts $fh "# V10S0 Submodule Self-Test IP Report"
puts $fh ""
...
puts $fh "- Candidate HDL root: `$hdl_dir`"
...
puts $fh "- Intended BD address: `0x43C10000`"
puts $fh "- Covered reusable RTL: frame packer, post-FFT bin-power/summary, band-power reducer, multi-lag correlation"
puts $fh ""
puts $fh "No bitstream or BOOT.bin is produced by this script."
close $fh

puts "V10S0_IP_REPORT=$report_file"
puts "V10S0_IP_ROOT=$ip_root"
