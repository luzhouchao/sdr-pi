...
set reports_dir [file join $repo_dir reports stage_v10s0_submodule_selftest_integration]
...
set cell_name p201_v10s0_selftest0
if {[llength [get_bd_cells -quiet /$cell_name]] > 0} {
    delete_bd_objs [get_bd_cells /$cell_name]
}
create_bd_cell -type ip -vlnv p201pro.local:user:p201_v10s0_submodule_selftest_axi_regs:1.0 $cell_name

# Keep the reusable submodule RTL visible to top-level synthesis even if an
# older cached IP snapshot omits a leaf file from its generated ipshared copy.
set direct_rtl_files [list]
set ip_component [file join $local_ip_repo p201_v10s0_submodule_selftest_axi_regs_1.0 component.xml]
...
foreach orphan_net [get_bd_nets -quiet p201_v10s0_selftest0_irq*] {
...
puts "V10S0_INTEGRATION_REPORT=$report_file"
